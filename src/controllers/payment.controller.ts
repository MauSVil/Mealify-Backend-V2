...
export const paymentController = {
  ...
  generateParams: async (req: RequestWithAuth, res: Response) => {
    try {
      const { email, amount, restaurant, userLatitude, userLongitude, clerkId, cart } = req.body;
      if (!email || !amount || !restaurant || !userLatitude || !userLongitude || !clerkId || !cart ) throw new Error('Missing required fields');
